function saveAveFigure(subjectID, subjectFolder, fileName, mainFigure,figureName, subjectFolderSave, customsave, customSavePath)
    if mainFigure ~= 9999
        if isa(subjectID, 'double')
            figureName = char(strcat(num2str(subjectID), {' '}, fileName(1:end-6), {'Ave - '}, figureName));
        else
            figureName = char(strcat(subjectID, {' '}, fileName(1:end-6), {'Ave - '}, figureName));
        end
