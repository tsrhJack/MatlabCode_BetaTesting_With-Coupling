...
for n = 1:Num_Subs(1,1)
   ...
   if strcmp(Thisside,'L') || strcmp(Thisside,'Left')
   ...
   elseif strcmp(Thisside,'R') || strcmp(Thisside,'Right')
   ...
   elseif strcmp(Thisside, 'B') || strcmp(Thisside,'Both')
...
if strcmp(AveFiles,'By Context')
    ...
    else if strcmp(AveFiles,'Across Sides')
        % use number and reason
        NewFileString = strcat(FileStringArray(:,1),FileStringArray(:,5));
...
for s=1:length(uniqindex)
    a=1;
    for m=1:size(SubjectALL_Vector,2)
        if strcmp(SubjectHeadersCat(m),Uniq(s))
    ...
    thisubtrials(:,s)=thisubcount;
    SubjectAves(:,s)=thissubave;
...
for s=1:length(uniqindex)
    ...
    thisubtrials2(:,s)=thisubcount2;
    SubjectAves2(s,:)=thissubave2;
end
end
...
%Sheet:Data - GDI Vectors
xlswrite(SaveFile,SubjectGaitHeaders2,'Data','C1');
%xlswrite(SaveFile,NumberCycles,'Sheet1','B10');
xlswrite(SaveFile,SubjectGDI_Vector,'Data','C11');
xlswrite(SaveFile,GDI_ColumnA,'Data','A11');
...
%Sheet:Variables - Outputvariables
...
xlswrite(SaveFile,GDI_ColumnA,'AveData','A11');
...
% Ask about where to save figures
    ...
    if customsave
        customSavePath = uigetdir(folderLocation);
    else
       customSavePath = ''; 
    end
...
for ii = 1:size(SubjectGDI_Vector,2)
    ii
    ...
    DataColumn = SubjectGDI_Vector(:,ii);
...
for ii = 1:size(SubjectAves,2)
    ...
    for j = 1:number_vector_code_pairs
        ...
        updateAverageVectorCodeOutput(outputVectorCode, header, currentVariable1, currentPlane1, currentVariable2,...
                                currentPlane2, bincounts)
    end
end
end
